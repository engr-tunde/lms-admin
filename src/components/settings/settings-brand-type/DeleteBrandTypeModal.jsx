...
import { deleteBrandType } from "../../../api";
import { errorNotification, successNotification } from "../../../utils/helpers";


const DeleteBrandTypeModal = ({ show, onClose, brandToDelete, brandToDeleteId, mutate }) => {
  if (!show) return null;

  const handleDelete = async () => {
    const response = await deleteBrandType(brandToDeleteId);
    if (response.status.toString().includes("20")) {
      successNotification(response.data?.message || "Brand type deleted");
      onClose();
      mutate();
    } else {
...
export default DeleteBrandTypeModal